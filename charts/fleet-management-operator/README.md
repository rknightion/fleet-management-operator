...
| controllers.crossNamespaceDiscoveryAuthz.enabled | bool | `false` | Enforce that a user creating a PipelineDiscovery or CollectorDiscovery with a spec.targetNamespace different from the CR's own namespace is itself authorized (via SubjectAccessReview at admission) to create Pipelines / Collectors in that target namespace. Closes the cross-namespace confused-deputy escalation where the operator's cluster-wide ServiceAccount would otherwise be borrowed to write CRs into any namespace. Default-allow (false) so existing installs see no behavior change until enabled. |
| controllers.externalAttributeSync.clusterWideSecretAccess | bool | `true` | Grant the operator cluster-wide read (get/list/watch) on Secrets, the auth material EAS sources reference. Set to false to drop the ClusterRole secret rule entirely when you instead provision namespaced Roles for the specific namespaces that hold EAS Secrets (least-privilege; see docs/security.md). True (default) preserves current behavior. |
...
| controllers.externalAttributeSync.secretLabelSelector | string | `""` | Optional label selector that scopes the operator's Secret informer cache (manager flag --external-source-secret-label-selector). When set (e.g. "fleetmanagement.grafana.com/external-source=true"), the operator only watches and reads Secrets carrying a matching label, shrinking the blast radius of its cluster-wide secret read and ensuring EAS can only use Secrets an admin has explicitly labelled. Empty (default) watches all Secrets (back-compat); existing installs must label their source Secrets before setting this. See docs/security.md. |
...
| controllers.pipeline.nameScope | string | `"none"` | Default Fleet pipeline name scope: "none" (use spec.name verbatim) or "namespace" (prefix the Fleet name with "<namespace>." so pipelines in different namespaces cannot collide on a shared Fleet name). A per-Pipeline fleetmanagement.grafana.com/name-scope annotation overrides this. Switching to "namespace" auto-migrates existing pipelines (delete-and-recreate); see docs/runbooks/pipeline-name-scope-migration.md. |
...
| controllers.tenantPolicy.enabled | bool | `false` | Enable TenantPolicy enforcement. When set, validating webhooks for Pipeline / RemoteAttributePolicy / ExternalAttributeSync / CollectorDiscovery require K8s subjects matched by a TenantPolicy CR to include at least one of the policy's required matchers. Default-allow when no policy matches; the CRD is always installed so policies can be authored ahead of enforcement. |
...
| rbac.userRoles.aggregateToDefaultRoles | bool | `false` | Also label the roles so they aggregate into the built-in Kubernetes admin/edit (editor) and view (viewer) cluster roles. SECURITY: leave this false. Aggregating editor into the built-in "edit" role grants every namespace admin in the cluster the ability to create these resources, re-opening the cross-namespace confused-deputy described in docs/security.md. Only enable in single-tenant clusters. |
| rbac.userRoles.create | bool | `false` | Render the <release>-editor and <release>-viewer ClusterRoles. |
